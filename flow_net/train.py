...
def train(args):
    ...
    log_file = '%s/%s/%s/training_log.txt' % (args.result_dir, args.log_dir, start_time)
    writer = SummaryWriter('%s/%s/%s' % (args.result_dir, args.log_dir, start_time))
    logger = setup_logger(log_file)
    print_args(args, logger)
    logger.info('[tensorboard] %s/%s/%s' % (args.result_dir, args.log_dir, start_time))
    ...
    for epoch in range(args.epochs + 1):
        ...
        with torch.set_grad_enabled(False):  # deactivate autograd to reduce memory usage
            ...
            end_time = time.time()
            time_used = (end_time - start_time) / 60.
            logger.info('>>> loss: {:.4f}, photometric_loss: {:.4f}, smoothness_loss: {:.4f}\n'
                        '    time used: {:.2f} mins'
            ...
            if best_loss > results['loss']:
                ...
                logger.info('>>> Model saved as {}... best loss {:.4f}'.format(model_dir, best_loss))
        # '''
        # break
        scheduler.step()
    writer.close()
...
def write_tensorboard(writer, results, epoch, progress, mode):
    ...
    for i in range(n):
        ...
        if len(events.shape) > 3:
            ...
            writer.add_images('%s/example%i/events' % (mode, i + 1), event_frame, epoch * 100 + progress,
                              dataformats='CHW')
...
def get_args():
    ...
    parser.add_argument('--result_dir', type=str, default='../exp_flow')
