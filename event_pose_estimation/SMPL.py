...
class SMPL(nn.Module):
    def __init__(self, model_path):
        ...
        np_J_regressor = np.array(model['J_regressor'].todense().T, dtype=float)
        ...
        np_weights = np.array(model['weights'], dtype=float)  # [6890, 24]
    ...
    def forward(self, beta, theta=None, get_skin=False, rotmats=None):
        ...
        weight = self.weight.repeat(num_batch, 1, 1)   # [N, 6890, 24]
