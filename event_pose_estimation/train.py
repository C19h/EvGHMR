...
from loss_funcs import compute_losses_, compute_mpjpe, compute_pa_mpjpe, compute_pelvis_mpjpe, \
...
def train(config):
    ...
    logger.info('[tensorboard] %s/%s' % (config['exper']['log_dir'], start_time))
    use_cuda = torch.cuda.is_available()
    device = torch.device("cuda:%s" % config['exper']['gpu_id'] if use_cuda else "cpu")
    dtype = torch.float32

    # set dataset
    dataset_train = CompleDataloader(
        config=config,
    ...
    total_iters = len(dataset_train) // config['exper']['batch_size']
    ...
    total_iters_val = len(dataset_val) // config['exper']['batch_size']
    ...
    if config['exper']['use_amp']:
        scaler = torch.cuda.amp.GradScaler()
    else:
        scaler = None

    # set optimizer
    optimizer = optim.Adam(model.parameters(), lr=config['exper']['lr_start'])
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=config['exper']['lr_decay_step'],
                                               gamma=config['exper']['lr_decay_rate'])
    ...
    save_dir = '%s/%s/%s' % (config['exper']['result_dir'], config['exper']['log_dir'], start_time)
    model_save_dir = '%s/%s/%s/model_events_pose.pkl' % (
        config['exper']['result_dir'], config['exper']['log_dir'], start_time)
    # training
    best_loss = 1e4
    for epoch in range(config['exper']['epochs']):
        logger.info(
            '====================================== Epoch %i ========================================' % (epoch + 1))
        # '''
        ...
        for iter, data in enumerate(train_generator):
            ...
            if config['exper']['use_amp']:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    loss_dict = compute_losses_(out, data, mse_func, device, config, model)
                    ...
                    # pve = torch.mean(torch.sqrt(torch.sum((out['verts'] - pa_verts) ** 2, dim=-1)),
                    #                  dim=-1)  # [B, T]
                    loss = loss_dict['tran'] + \
                           loss_dict['theta'] + \
                           loss_dict['shape'] + \
            ...
            results['scalar/shape'].append(loss_dict['shape'].detach())
            ...
            #     break
            # if iter % 2 == 0:
            tran = loss_dict['tran'].item()
            theta = loss_dict['theta'].item()
            shape = loss_dict['shape'].item()
            joints3d = loss_dict['joints3d'].item()
            joints2d = loss_dict['joints2d'].item()
            flow = loss_dict['flow'].item()
            print(
                f'iter: {iter}, loss_sum: {loss.item():.4g}, tran: {tran:.4g}, theta: {theta:.4g}, shape: {shape:.4g}, joint3d: {joints3d:.4g}, joint2d: {joints2d:.4g}, flow: {flow:.4g}')
            display = 1
            if iter != 0 and iter % (total_iters // display) == 0:
                ...
                results['shape'] = torch.mean(torch.stack(results['scalar/shape'], dim=0))
                ...
                results['cam_intr'] = out['cam_intr'][0].detach()
                ...
                write_tensorboard(writer, results, epoch + 1, progress - 1, 'train', config)

                end_time = time.time()
                time_used = (end_time - start_time) / 60.
                logger.info('>>> [epoch {:2d}/ iter {:6d}] {:3d}%\n'
                            '    loss {:.4f}, tran {:.4f}, theta {:.4f}, shape {:.4f}, joints3d {:.4f}, joints2d {:.4f} \n'
                            '    flow {:.4f}, \n'
                            ...
                            .format(epoch, iter, progress - 1, results['loss'], results['tran'], results['theta'],
                                    ...
                                    optimizer.param_groups[0]['lr'], time_used, (100 / progress - 1) * time_used))
                logger.info(
                      '    mpjpe {}\n'
                      '    pa_mpjpe {}\n'
                      '    pel_mpjpe {}\n'
        ...
        with torch.set_grad_enabled(False):  # deactivate autograd to reduce memory usage
            for iter, data in enumerate(val_generator):
                ...
                if iter != 0 and iter % (total_iters_val // display) == 0:
                    ...
                    results['grays'] = data['grays_o'][0].detach()
                    results['scene'] = data['scene'][0].detach()
                    write_tensorboard(writer, results, epoch + 1, iter // 100, 'test', config)
            ...
            end_time = time.time()
            time_used = (end_time - start_time) / 60.
            logger.info('>>> loss {:.4f}, tran {:.4f}, theta {:.4f}, shape {:.4f}, joints3d {:.4f}, joints2d {:.4f} \n'
                        '    flow {:.4f}, time used: {:.2f} mins \n'

                        .format(results['loss'], results['tran'], results['theta'], results['shape'],
                                results['joints3d'], results['joints2d'],
                                results['flow'], time_used, 1000 * results['mpjpe']))
            ...
            if best_loss > results['loss']:
                ...
                logger.info('>>> Model saved as {}... best loss {:.4f}'.format(model_save_dir, best_loss))
            # break
        # '''
        scheduler.step()
    writer.close()
...
def write_tensorboard(writer, results, epoch, progress, mode, config):
    ...
    com = np.concatenate([ef[:, 4:7, ...], render_imgs/255.0], axis=3)
...
def get_config():
    ...
    parser.add_argument('--config', type=str, default='./train_compl.yaml')
