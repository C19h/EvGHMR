...
from event_pose_estimation.geometry import projection_torch, batch_compute_similarity_transform_torch, mat2rotvec
...
def compute_pelvis_mpjpe(pred, target):
    ...
    pred = pred - pred_pel
    target_pel = (target[:, :, left_heap_idx:left_heap_idx + 1, :] + target[:, :, right_heap_idx:right_heap_idx + 1,
                                                                     :]) / 2
...
def compute_losses_(out, target, mse_func, device, config, model):
    losses = {}
    if config['exper']['tran_loss'] > 0:
        tran_loss = mse_func(out['tran'], target['tran'])
        losses['tran'] = config['exper']['tran_loss'] * tran_loss
    else:
        losses['tran'] = torch.tensor([0], device=device).float()

    if 'shape' in out.keys() and config['exper']['shape_loss'] > 0:
        shape_loss = mse_func(out['shape'], target['shape'])
        losses['shape'] = config['exper']['shape_loss'] * shape_loss
    else:
        losses['shape'] = torch.tensor([0], device=device).float()

    if config['exper']['theta_loss'] > 0:
        B, T, _ = target['theta'].size()
        target_rotmats = batch_rodrigues(target['theta'].view(-1, 3)).view(B, T, 24, 3, 3)

        if config['exper']['use_geodesic_loss']:
            eps = 1e-6
        ...
        losses['theta'] = config['exper']['theta_loss'] * theta_loss
    else:
        losses['theta'] = torch.tensor([0], device=device).float()

    if config['exper']['joints3d_loss'] > 0:
        joints3d_loss = mse_func(out['joints3d'], target['joints3d'])
        losses['joints3d'] = config['exper']['joints3d_loss'] * joints3d_loss
    else:
        losses['joints3d'] = torch.tensor([0], device=device).float()

    if config['exper']['joints2d_loss'] > 0:
        ...
        losses['joints2d'] = config['exper']['joints2d_loss'] * joints2d_loss
    else:
        losses['joints2d'] = torch.tensor([0], device=device).float()

    if config['exper']['flow_loss'] > 0 and config['exper']['flow_loss']:
        flow_loss = compute_flow_loss(out['verts'], target['flows'], out['cam_intr'], device)
        losses['flow'] = config['exper']['flow_loss'] * flow_loss
        # losses['flow'] = torch.tensor([0], device=device).float()
    elif config['exper']['denseflow_loss'] > 0 and config['exper']['denseflow_loss']:
        flow_loss = compute_denseflow_loss_all(out, target['flows'], device, model.smpl)
        losses['flow'] = config['exper']['denseflow_loss'] * flow_loss
...
def compute_flow_loss(verts, pred_flows, cam_intr, device):
    ...
    _, T_v, _, _ = verts.size()
    verts_2d = projection_torch(verts, cam_intr)  # [B, T, 6890, 2]
    verts_flow = verts_2d[:, 1:, :, :] - verts_2d[:, :-1, :, :]  # [B, T-1, 6890, 2] <t+1> - <t>
    ...
    _flows = pred_flows[:, 1:, ...].contiguous().reshape(-1, 2, H, W)
    T = T - 1

    sampled_flow = F.grid_sample(_flows.to(flow_indices.dtype), flow_indices)  # [BT, 2, H, W]  (u, v)
    ...
    valid = torch.norm(sampled_flow, p=2, dim=-1) > 3
    # epsilon = 1e-8
    # epe_loss = (verts_flow - sampled_flow) ** 2
    # epe_loss = torch.sum(epe_loss, dim=-1)
    # epe_loss = torch.sqrt(epe_loss + epsilon) * valid
    # loss_epe = torch.mean(epe_loss)

    sim = valid * (1 - F.cosine_similarity(verts_flow, sampled_flow, dim=3))  # [B, T, 6890]
    loss_sim = torch.mean(sim)
    return loss_sim


def compute_denseflow_loss_all(out, pred_flows, device, smpl):
    # verts: [B, T, 6890, 3]
    # pred_flows: [B, T, 2, H, W]
    verts = out['verts']
    cam_intr = out['cam_intr']
    B, T, _, H, W = pred_flows.size()


    # idx = np.random.randint(0, B)
    # verts = verts[idx].unsqueeze(0)
    # cam_intr = cam_intr[idx].unsqueeze(0)
    # pred_flows = pred_flows[idx].unsqueeze(0)
    # B = 1

    verts_2d = projection_torch(verts, cam_intr)  # [B, T, 6890, 2]
    verts_flow = verts_2d[:, 1:, :, :] - verts_2d[:, :-1, :, :]  # [B, T-1, 6890, 2] <t+1> - <t>

    faces = torch.tensor(smpl.faces.astype(np.int32)).repeat(B * (T - 1), 1, 1).type_as(verts)
    cam_intr = cam_intr[:, 1:, :]
    fx, fy, cx, cy = cam_intr[..., 0], cam_intr[..., 1], cam_intr[..., 2], cam_intr[..., 3]

    K = torch.zeros(B, T-1, 3, 3, dtype=torch.float32)
    K[..., 0, 0] = fx
    K[..., 1, 1] = fy
    K[..., 0, 2] = cx
    K[..., 1, 2] = cy
    ...
    cameras = cameras_from_opencv_projection(
        R=torch.eye(3).repeat(B * (T - 1), 1, 1).type_as(verts),
        tvec=torch.zeros(B * (T - 1), 3).type_as(verts),
        camera_matrix=K.type_as(verts),
        image_size=torch.tensor([H, W]).expand(B * (T - 1), 2).type_as(verts)).to(verts.device)
    verts_rgb = torch.ones_like(smpl.v_template).repeat(B * (T - 1), 1, 1).type_as(verts)
    textures = TexturesVertex(verts_rgb)
    # set the verts requires_grad=False, or will cause the gradient exploding
    mesh = Meshes(
        verts=verts[:, :-1, ...].contiguous().reshape(B * (T - 1), verts.size(-2), verts.size(-1)).detach(),
        faces=faces,
        textures=textures)
    renderer = create_renderer(image_size=[H, W])
    ...
    flow_x_fw = torch.gather(verts_flow[..., 0], 2,
                             verts_index.reshape(B, T - 1, -1).long())
    flow_y_fw = torch.gather(verts_flow[..., 1], 2,
                             verts_index.reshape(B, T - 1, -1).long())
    flow_x_fw = flow_x_fw.reshape(B, T - 1, H, W, 3)
    flow_y_fw = flow_y_fw.reshape(B, T - 1, H, W, 3)
    flow_xy_fw = torch.stack([flow_x_fw, flow_y_fw], dim=-1)

    # get the mean flow
    bary_flow_weights = frags.bary_coords[:, :, :, 0, :].reshape(B, T - 1, H, W, 3, 1)
    mask = mask.reshape(B, T - 1, H, W, 1)
    flows_fw = torch.sum(flow_xy_fw * bary_flow_weights * mask[..., None, :], dim=-2)

    flows_fw = flows_fw.reshape(-1, H, W, 2)

    _flows = pred_flows[:, 1:, ...].contiguous().reshape(-1, 2, H, W).permute(0, 2, 3, 1)
    valid = torch.norm(_flows, p=2, dim=-1) > 3
    epsilon = 1e-8
    epe_loss = (flows_fw - _flows) ** 2
    epe_loss = torch.sum(epe_loss, dim=-1)
    epe_loss = torch.sqrt(epe_loss + epsilon) * valid
    loss = torch.mean(epe_loss)
    # sim = valid * (1 - F.cosine_similarity(flows_fw, sampled_flow, dim=4))  # [B, T, 6890]
    # loss= torch.mean(sim)
    return loss
